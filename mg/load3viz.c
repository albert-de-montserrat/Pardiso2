...
/* Load in Binary Format ---------------------------- */
	...
	hid_t       file_id, group_id, attr_id, dataset_id, dataspace_id, memtype, memspace, filetype, space,npoints;  /* identifiers */
        ...
        file_id = H5Fopen(fl1in, H5F_ACC_RDONLY, H5P_DEFAULT);
	/* General */
...
/* Add Eulerian field ------------------------------------------------*/
void add_eulerian_to_XDMF(char *field_name,char *file_name,char *dataset_id,int is_vector,long int dimension,FILE *fl)
{
int d;
char *type_vec_scal;
/**/
...
{
memtot[0]=0;
memtot[0]+=xnumx*ynumy*znumz*8;
memtot[0]+=(xnumx+ynumy+znumz)*10*8;
...
cp=malloc(sizeof(float) * nodenum);
kt=malloc(sizeof(float) * nodenum);
ht=malloc(sizeof(float) * nodenum);
et=malloc(sizeof(float) * nodenum);
...
sol=malloc(sizeof(float) * nodenum);
memtot[0]+=4*24*nodenum;
}
if(mode==2)
{
/* Markers */
...
markk=malloc(sizeof(float) * marknum0);
markd=malloc(sizeof(float) * marknum0);
markw=malloc(sizeof(float) * marknum0);
marke=malloc(sizeof(float) * marknum0);
markt=malloc(sizeof(char) * marknum0);
memtot[0]+=marknum0*(6*4+1);
/* Rock properties*/
...
free(pr);free(nu);free(ro);free(fd);
free(cp);free(kt);free(ht);free(et);free(tk);
...
free(markk);free(markd);free(markw);free(markt);free(marke);
}
/**/
/* Memory for voronoi tessellation */
if(mode==4)
{
Lc=calloc(6*marknum,sizeof(long int));
lt=calloc(6*marknum,sizeof(long int));
gxc=calloc(xnum,sizeof(double));
gyc=calloc(ynum,sizeof(double));
gzc=calloc(znum,sizeof(double));
idx=calloc(nvrtx,sizeof(int));
Idx=calloc(nvrtx,sizeof(int));
Idx2=calloc(nvrtx,sizeof(int));
Idxc=calloc(ncell,sizeof(int));
Idxv=calloc(nvrtx,sizeof(long int));
Idxv2=calloc(nvrtx,sizeof(char));
/* Vertices of voronoi cells*/
/*
vrtx=malloc(sizeof(float *) * nvrtx);
for(i=0;i<nvrtx;i++)
        {
        vrtx[i]=malloc(sizeof(float) * 3);
	}
*/
/* Connectivity for voronoi cells*/
/*
cntv=malloc(sizeof(long int *) * ncell);
for(i=0;i<ncell;i++)
        {
        cntv[i]=malloc(sizeof(long int) * 8);
	}
*/
}
/* Free memory for voronoi tessellation */
if(mode==5)
{
free(idx);   
free(Idx);   
