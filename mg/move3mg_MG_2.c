...
/* mgi - cur multigrid level */
{
    ...
    /*
     int n1;
     if(m1==mgx[mgi]-1 && m3==mgz[mgi]-1){for(n1=0;n1<=un1[0];n1++)printf("Exx %ld %ld %ld %d %e %ld \n",m1,m2,m3,n1,ui1[n1],un1[n1]);getchar();}
     */
    return 0;
}
/* Left side or Value for Sxx  Equation */
...
/* ynval - Val Syz Calc Y(0)/N(koefficient) */
{
    ...
    //if (m3==0 && m1==0) {printf("asfjk %i",v[0]);getchar();}
    v[2]=v[0]+mgy[mgi];v[3]=v[2]+1;
    if(v[2]<0) {
      printf(" \n%i %i %i %i %i %i %i ",m1,m2,m3,mgi,mgp[mgi],mgxy[mgi],mgy[mgi]);
      printf("\n v[2] %i v[0] %i mgy[mgi] %i",v[2],v[0],mgy[mgi]);getchar();
    }
    v[4]=v[0]+mgxy[mgi];v[5]=v[4]+1;
    v[6]=v[4]+mgy[mgi];v[7]=v[6]+1;
    if(xperiodic && m1==0)
    ...
    if(xperiodic && m1==0 && zperiodic && m3==0)
    {
        ...
        //v[2]=mgp[mgi]+(mgz[mgi]-2)*mgxy[mgi]+(mgx[mgi]-1)*mgy[mgi]+m2;
        v[4]=mgp[mgi]+(mgx[mgi]-2)*mgy[mgi]+m2;
        //v[4]=mgp[mgi]+(mgz[mgi]-1)*mgxy[mgi]+(mgx[mgi]-2)*mgy[mgi]+m2;
        v[6]=mgp[mgi]+m2; v[7]=v[6]+1;
    }
    /**/
    /**/
    /**/
    ...
    if(ynval==0)
    {
        ...
        leftsxz=dvxdz+dvzdx;
        /**/
        //if(xperiodic && m1==0 && zperiodic && m3==0){printf("%ld %ld %ld %e %e %e %e %e %e\n",m1,m2,m3,vx[v[6]],vx[v[2]],vz[v[6]],vz[v[4]],dvxdz,dvzdx); getchar();}
        /* Save Exy */
        eps1[0]=leftsxz;
        /**/
        /* Calc Sxy=2Nu*Exy */
    ...
    for(n1=0;n1<=un1[0];n1++){
      if(un1[n1]<0){
      printf("\n negative little cunt at %i %i %i \n",n1,un1[n1],un1[0]);
      printf("\n v[2] %i v[4] %i v[6] %i \n",v[2],v[4],v[6]);getchar();
    }
    }
    /**/
    ...
    /*
     ...
     int n1;
     if(xperiodic && m1==mgx[mgi]-1 && zperiodic && m3==0)
     {for(n1=0;n1<=un1[0];n1++)printf("Exx %ld %ld %ld %d %e %ld \n",m1,m2,m3,n1,ui1[n1],un1[n1]);getchar();}
     */
    return 0;
}
/* Left side or Value for Sxz  Equation */
...
/* ynerr - Err Calc Y(1)/N(0) */
{
    ...
    if(spheryn)
    {
        xkf=mggx[mgi][m1]-xsize/2.0;
        ykf=(mggy[mgi][m2]+mggy[mgi][m2+1]-ysize)/2.0;
        zkf=(mggz[mgi][m3]+mggz[mgi][m3+1]-zsize)/2.0;
        skf=pow(xkf*xkf+ykf*ykf+zkf*zkf,0.5);
        if(skf>0)
        {
            GXKOEFX=-xkf/skf*pow(GXKOEF*GXKOEF+GYKOEF*GYKOEF+GZKOEF*GZKOEF,0.5);
        }
        else
        {
            GXKOEFX=0;
    ...
    if(xperiodic && m1==0){
      xkf=2.0/(mggx[mgi][1]-mggx[mgi][0]+mggx[mgi][mgx[mgi]-1]-mggx[mgi][mgx[mgi]-2]);
	        //printf("%f %f %f %f %f \n",mggx[mgi][1],mggx[mgi][0],mggx[mgi][mgx[mgi]-1],mggx[mgi][mgx[mgi]-2],xkf);getchar();
     }
    ...
    v[4]=v[0]+mgxy[mgi];v[5]=v[4]+1;
    v[6]=v[4]+mgy[mgi];v[7]=v[6]+1;
    if(xperiodic && m1==0) v[5]=mgp[mgi]+(m3+1)*mgxy[mgi]+(mgx[mgi]-1)*mgy[mgi]+m2+1;
    /**/
    /**/
    /**/
    ...
    // printf("rightx %ld %ld %ld %ld",ro[v[0]],ro[v[1]],ro[v[4]],ro[v[5]]);getchar();
    /**/
    /**/
    /**/
    /* Solution koef calc */
    ...
    /*  6  5                                     */
    /* dSIGxx/dX */
    if(xperiodic && m1==0) sxxcalc(mgx[mgi]-1,m2+1,m3+1,-xkf,mgi,eps1,un1,ui1);
    else sxxcalc(m1  ,m2+1,m3+1,-xkf,mgi,eps1,un1,ui1);
    ...
    ui1[10]+=ui1[21];
    un1[0]=17;
    if(yperiodic && m2==mgy[mgi]-2) sxycalc(m1  ,0,m3  , ykf,mgi,eps1,un1,ui1);
    else sxycalc(m1  ,m2+1,m3  , ykf,mgi,eps1,un1,ui1);
    ui1[ 1]+=ui1[18];
    ...
    if (un1[6]<0) {
      printf(" (problem in sxzcalc) %i %i %i\n",m1,m2,m3);getchar();
    }
    ui1[12]+=ui1[20];
    ui1[14]+=ui1[21];
    un1[0]=17;
    if(zperiodic && m3==mgz[mgi]-2) sxzcalc(m1  ,m2  ,0, zkf,mgi,eps1,un1,ui1);
    else sxzcalc(m1  ,m2  ,m3+1, zkf,mgi,eps1,un1,ui1);
    ui1[ 1]+=ui1[18];
    ...
    {
        un1[0]=17;
        if(xperiodic && m1==0) continadd(mgx[mgi]-1,m2+1,m3+1,-xkf*p2koef,mgi,eps1,un1,ui1);
        else continadd(m1  ,m2+1,m3+1,-xkf*p2koef,mgi,eps1,un1,ui1);
        ui1[ 2]+=ui1[18];
        ui1[ 1]+=ui1[19];
        ui1[ 8]+=ui1[20];
        ui1[ 9]+=ui1[21];
    ...
    un1[0]=17;
    int nn1;
    for(nn1=1;nn1<=un1[0];nn1++){
       if(un1[nn1]<1){
        printf("nu1[m1] at it = %i",nn1);getchar();
       }
    }
    /**/
    /**/
    /**/
    /*
     if(m1==0 && m2==1 && m3==0){for(n1=0;n1<=un1[0];n1++)printf("Vx %d %e %ld \n",n1,ui1[n1],un1[n1]);getchar();}
     */
     // if(mgi==multinum){for(n1=0;n1<=un1[0];n1++)printf("Vx %d %e %ld \n",n1,ui1[n1],un1[n1]);getchar();}
    return 0;
}
/* Left+Right Side or Err for X-Stokes Equation */
...
/* ynerr - Err Calc Y(1)/N(0) */
{
    ...
    if(spheryn)
    {
        xkf=(mggx[mgi][m1]+mggx[mgi][m1+1]-xsize)/2.0;
        ykf=mggy[mgi][m2]-ysize/2.0;
        zkf=(mggz[mgi][m3]+mggz[mgi][m3+1]-zsize)/2.0;
        skf=pow(xkf*xkf+ykf*ykf+zkf*zkf,0.5);
        if(skf>0)
        {
            GYKOEFY=-ykf/skf*pow(GXKOEF*GXKOEF+GYKOEF*GYKOEF+GZKOEF*GZKOEF,0.5);
        }
        else
        {
            GYKOEFY=0;
    ...
    if(yperiodic && m2==0) ykf=2.0/(mggy[mgi][1]-mggy[mgi][0]+mggy[mgi][mgy[mgi]-1]-mggy[mgi][mgy[mgi]-2]);
    zkf=1.0/(mggz[mgi][m3+1]-mggz[mgi][m3]);
    /**/
    /**/
    /**/
    ...
    v[4]=v[0]+mgxy[mgi];v[5]=v[4]+1;
    v[6]=v[4]+mgy[mgi];v[7]=v[6]+1;
    if(yperiodic && m2==0) v[6]=mgp[mgi]+(m3+1)*mgxy[mgi]+(m1+1)*mgy[mgi]+mgy[mgi]-1;
    /**/
    /**/
    /**/
    ...
    /* dSIGxx/dX */
    /* dSIGxx/dX */
    if(yperiodic && m2==0) syycalc(m1+1,mgy[mgi]-1,m3+1,-ykf,mgi,eps1,un1,ui1);
    else syycalc(m1+1,m2  ,m3+1,-ykf,mgi,eps1,un1,ui1);
    ...
    ui1[ 1]+=ui1[21];
    un1[0]=17;
    if(xperiodic && m1==mgx[mgi]-2) sxycalc(0,m2  ,m3  , xkf,mgi,eps1,un1,ui1);
    else sxycalc(m1+1,m2  ,m3  , xkf,mgi,eps1,un1,ui1);
    ui1[10]+=ui1[18];
    ui1[11]+=ui1[19];
    ui1[ 1]+=ui1[20];
    ...
    ui1[13]+=ui1[21];
    un1[0]=17;
    if(zperiodic && m3==mgz[mgi]-2) syzcalc(m1  ,m2  ,0, zkf,mgi,eps1,un1,ui1);
    else syzcalc(m1  ,m2  ,m3+1, zkf,mgi,eps1,un1,ui1);
    ui1[ 1]+=ui1[18];
    un1[ 7]=un1[19]; ui1[ 7] =ui1[19];
    ui1[14]+=ui1[20];
    ui1[15]+=ui1[21];
    ...
    {
        un1[0]=17;
        if(yperiodic && m2==0)    continadd(m1+1,mgy[mgi]-1,m3+1,-ykf*p2koef,mgi,eps1,un1,ui1);
        else    continadd(m1+1,m2  ,m3+1,-ykf*p2koef,mgi,eps1,un1,ui1);
        ui1[ 8]+=ui1[18];
        ui1[10]+=ui1[19];
        ui1[ 4]+=ui1[20];
        ui1[ 1]+=ui1[21];
...
/* ynerr - Err Calc Y(1)/N(0) */
{
    ...
    if(spheryn)
    {
        xkf=(mggx[mgi][m1]+mggx[mgi][m1+1]-xsize)/2.0;
        ykf=(mggy[mgi][m2]+mggy[mgi][m2+1]-ysize)/2.0;
        zkf=mggz[mgi][m3]-zsize/2.0;
        skf=pow(xkf*xkf+ykf*ykf+zkf*zkf,0.5);
        if(skf>0)
        {
            GZKOEFZ=-zkf/skf*pow(GXKOEF*GXKOEF+GYKOEF*GYKOEF+GZKOEF*GZKOEF,0.5);
        }
        else
        {
            GZKOEFZ=0;
    ...
    if(zperiodic && m3==0) zkf=2.0/(mggz[mgi][1]-mggz[mgi][0]+mggz[mgi][mgz[mgi]-1]-mggz[mgi][mgz[mgi]-2]);
    /**/
    /**/
    /**/
    /*   Z=0      Z=1 */
    ...
    v[4]=v[0]+mgxy[mgi];v[5]=v[4]+1;
    v[6]=v[4]+mgy[mgi];v[7]=v[6]+1;
    if(zperiodic && m3==0) v[3]=mgp[mgi]+(mgz[mgi]-1)*mgxy[mgi]+(m1+1)*mgy[mgi]+m2+1;
    /**/
    /**/
    /**/
    /* RIGHT parts of X-Stokes */
    ...
    /*  6  5         9                           */
    /* dSIGxx/dX */
    if(zperiodic && m3==0) szzcalc(m1+1,m2+1,mgz[mgi]-1,-zkf,mgi,eps1,un1,ui1);
    else szzcalc(m1+1,m2+1,m3  ,-zkf,mgi,eps1,un1,ui1);
    ...
    ui1[ 1]+=ui1[21];
    un1[0]=17;
    if(xperiodic && m1==mgx[mgi]-2) sxzcalc(0,m2  ,m3  , xkf,mgi,eps1,un1,ui1);
    else sxzcalc(m1+1,m2  ,m3  , xkf,mgi,eps1,un1,ui1);
    ui1[14]+=ui1[18];
    ui1[15]+=ui1[19];
    ui1[ 1]+=ui1[20];
    un1[ 3]=un1[21]; ui1[ 3] =ui1[21];
    ...
    ui1[ 1]+=ui1[21];
    un1[0]=17;
    if(yperiodic && m2==mgy[mgi]-2) syzcalc(m1  ,0,m3  , ykf,mgi,eps1,un1,ui1);
    else syzcalc(m1  ,m2+1,m3  , ykf,mgi,eps1,un1,ui1);
    ui1[ 9]+=ui1[18];
    ui1[11]+=ui1[19];
    ui1[ 1]+=ui1[20];
    un1[ 5]=un1[21]; ui1[ 5] =ui1[21];
    ...
    {
        un1[0]=17;
        if(zperiodic && m3==0) continadd(m1+1,m2+1,mgz[mgi]-1,-zkf*p2koef,mgi,eps1,un1,ui1);
        else continadd(m1+1,m2+1,m3  ,-zkf*p2koef,mgi,eps1,un1,ui1);
        ui1[12]+=ui1[18];
        ui1[14]+=ui1[19];
        ui1[ 8]+=ui1[20];
        ui1[ 9]+=ui1[21];
    ...
    /**/
    /*
     if(m3==0){for(n1=0;n1<=un1[0];n1++)printf("Exx %ld %ld %ld %d %e %ld \n",m1,m2,m3,n1,ui1[n1],un1[n1]);getchar();}
     for(n1=0;n1<=un[0];n1++)printf("Vz %e %ld  %e %e %e\n",ui[n1],un[n1],xkf,ykf,zkf);getchar();
     if(m1==0 && m3==0){for(n1=0;n1<=un1[0];n1++)printf("Exx %ld %ld %ld %d %e %ld \n",m1,m2,m3,n1,ui1[n1],un1[n1]);getchar();}
     */
    return 0;
}
/* Left+Right Side or Err for Z-Stokes Equation */
...
/* ynerr - Err Calc Y(1)/N(0) */
{
    ...
    zkf=1.0/(mggz[mgi][m3]-mggz[mgi][m3-1]);
    // if(mgi==multinum){
    //   printf("%e %e %e | %i %i %i | %i\n",xkf,ykf,zkf,m1,m2,m3,mgi);
    //   printf("%e %e \n",mggx[mgi][m1],mggx[mgi][m1-1]);
    //   printf("%e %e \n",mggy[mgi][m1],mggy[mgi][m1-1]);
    //   printf("%e %e \n",mggz[mgi][m1],mggz[mgi][m1-1]);
    //   getchar();
    // }
    /**/
    /**/
...
/* Left side or Err for Boundary Condition Equation */
/* Ai=CONST+KOEF*An */
double bonderrpar(long int mcmax, long int mcmax2, int ynerr, double *eps1, long int *un1, double *ui1)
/* mcmax - numer of cur Vx in sol[] */
/* ynerr - Err Calc Y(1)/N(0) */
{
    /* Val Buffer */
    ...
    {
        /* Add Const */
        leftb=sol0[mcmax]-bondv1[bondm[mcmax2]][0];
        /* Add Koef */
        if(bondn1[bondm[mcmax2]])
        {
            leftb-=bondv1[bondm[mcmax2]][1]*sol0[bondn1[bondm[mcmax2]]-1 - mgp[multinum]*4];
        }
        /*
         printf("Bo %ld %e %e \n",mcmax,sol0[mcmax],leftb);
         */
    ...
    /* Add Y CONST */
    un1[0]=1;
    ui1[0]=bondv1[bondm[mcmax2]][0];
    un1[1]=mcmax;
    ui1[1]=1.0;
    {if(un1[un1[0]]<1) printf(" 1: %ld %ld %ld \n",mcmax,mcmax2,un1[un1[0]]);}
    /* Add Y PAR1,PAR2,PAR3 */
    if(bondn1[bondm[mcmax2]])
    {
        un1[0]+=1;
        un1[un1[0]]=bondn1[bondm[mcmax2]]-1 - mgp[multinum]*4;
        if(un1[un1[0]]<1){
            printf(" 2: %ld %ld %ld \n",mcmax,mcmax2,un1[un1[0]]);
            printf("%i %i \n",bondn1[bondm[mcmax2]]-1,mgp[multinum]*4); getchar();
        }
        ui1[un1[0]]=-bondv1[bondm[mcmax2]][1];
    }
    /*
     printf("Bo %ld %e %ld %e \n",mcmax,bondv1[mcmax][0],bondn1[mcmax],bondv1[mcmax][1]);getchar();
     printf("Bo %ld %e %e \n",mcmax,sol0[mcmax],leftb);getchar();
...
{
    /* Counters */
    long int m2prn=0,m1prn=0,m1,m2,m3,m4,m5,m10,m20,m30,m40,mcmax,mcmax0,mcmax1,mcmax2,nnodes;
    ...
    int i,nt,mgi=0,n0,n1,n2,n3,ynstoks=1,multinum0=multinum,multcur=0;
    long int v[8];
...
#pragma omp for schedule(static)
            ...
            for (m1=0;m1<mgx[mgi];m1++)
                for (m2=0;m2<mgy[mgi];m2++)
                {
                    ...
                    if(xperiodic && !yperiodic)
                    {
                        ...
                        {
                            if(m1==mgx[mgi]-1) {sxy[mcmax1]=sxycalc(0,m2,m3,0,mgi,eps1,un1,ui1); exy[mcmax1]=eps1[0];}
                            else {sxy[mcmax1]=sxycalc(m1,m2,m3,0,mgi,eps1,un1,ui1); exy[mcmax1]=eps1[0];}
                        }
                    }
                    else if(yperiodic && !xperiodic)
                    {
                        ...
                        {
                            if(m2==mgy[mgi]-1) {sxy[mcmax1]=sxycalc(m1,0,m3,0,mgi,eps1,un1,ui1); exy[mcmax1]=eps1[0];}
                            else {sxy[mcmax1]=sxycalc(m1,m2,m3,0,mgi,eps1,un1,ui1); exy[mcmax1]=eps1[0];}
                        }
                    }
                    else if(xperiodic && yperiodic)
                    {
                        if(m3<mgz[mgi]-1)
                        {
                            if(m1==mgx[mgi]-1 && m2==mgy[mgi]-1) {sxy[mcmax1]=sxycalc(0,0,m3,0,mgi,eps1,un1,ui1); exy[mcmax1]=eps1[0];}
                            else if(m1==mgx[mgi]-1) {sxy[mcmax1]=sxycalc(0,m2,m3,0,mgi,eps1,un1,ui1); exy[mcmax1]=eps1[0];}
                            else if(m2==mgy[mgi]-1) {sxy[mcmax1]=sxycalc(m1,0,m3,0,mgi,eps1,un1,ui1); exy[mcmax1]=eps1[0];}
                            else {sxy[mcmax1]=sxycalc(m1,m2,m3,0,mgi,eps1,un1,ui1); exy[mcmax1]=eps1[0];}
                        }
                    }
                    else
                    ...
                    if(xperiodic && !zperiodic)
                    {
                        ...
                        {
                            if(m1==mgx[mgi]-1) {sxz[mcmax1]=sxzcalc(0,m2,m3,0,mgi,eps1,un1,ui1); exz[mcmax1]=eps1[0];}
                            else {sxz[mcmax1]=sxzcalc(m1,m2,m3,0,mgi,eps1,un1,ui1); exz[mcmax1]=eps1[0];}
                        }
                    }
                    else if(zperiodic && !xperiodic)
                    {
                        ...
                        {
                            if(m3==mgz[mgi]-1) {sxz[mcmax1]=sxzcalc(m1,m2,0,0,mgi,eps1,un1,ui1); exz[mcmax1]=eps1[0];}
                            else {sxz[mcmax1]=sxzcalc(m1,m2,m3,0,mgi,eps1,un1,ui1); exz[mcmax1]=eps1[0];}
                        }
                    }
                    else if(xperiodic && zperiodic)
                    {
                        if(m2<mgy[mgi]-1)
                        {
                            if(m1==mgx[mgi]-1 && m3==mgz[mgi]-1) {sxz[mcmax1]=sxzcalc(0,m2,0,0,mgi,eps1,un1,ui1); exz[mcmax1]=eps1[0];}
                            else if(m1==mgx[mgi]-1) {sxz[mcmax1]=sxzcalc(0,m2,m3,0,mgi,eps1,un1,ui1); exz[mcmax1]=eps1[0];}
                            else if(m3==mgz[mgi]-1) {sxz[mcmax1]=sxzcalc(m1,m2,0,0,mgi,eps1,un1,ui1); exz[mcmax1]=eps1[0];}
                            else {sxz[mcmax1]=sxzcalc(m1,m2,m3,0,mgi,eps1,un1,ui1); exz[mcmax1]=eps1[0];}
                            //if(m3==0 && m1==0){printf("%ld %ld %ld %e %e\n",m1,m2,mcmax1,sxz[mcmax1],exz[mcmax1]); getchar();}
                        }
                    }
                    else
                    {
                    ...
                    if(yperiodic && !zperiodic)
                    {
                        ...
                        {
                            if(m2==mgy[mgi]-1) {syz[mcmax1]=syzcalc(m1,0,m3,0,mgi,eps1,un1,ui1); eyz[mcmax1]=eps1[0];}
                            else {syz[mcmax1]=syzcalc(m1,m2,m3,0,mgi,eps1,un1,ui1); eyz[mcmax1]=eps1[0];}
                        }
                    }
                    else if(zperiodic && !yperiodic)
                    {
                        ...
                        {
                            if(m3==mgz[mgi]-1) {syz[mcmax1]=syzcalc(m1,m2,0,0,mgi,eps1,un1,ui1); eyz[mcmax1]=eps1[0];}
                            else {syz[mcmax1]=syzcalc(m1,m2,m3,0,mgi,eps1,un1,ui1); eyz[mcmax1]=eps1[0];}
                        }
                    }
                    else if(yperiodic && zperiodic)
                    {
                        if(m1<mgx[mgi]-1)
                        {
                            if(m2==mgy[mgi]-1 && m3==mgz[mgi]-1) {syz[mcmax1]=syzcalc(m1,0,0,0,mgi,eps1,un1,ui1); eyz[mcmax1]=eps1[0];}
                            else if(m2==mgy[mgi]-1) {syz[mcmax1]=syzcalc(m1,0,m3,0,mgi,eps1,un1,ui1); eyz[mcmax1]=eps1[0];}
                            else if(m3==mgz[mgi]-1) {syz[mcmax1]=syzcalc(m1,m2,0,0,mgi,eps1,un1,ui1); eyz[mcmax1]=eps1[0];}
                            else {syz[mcmax1]=syzcalc(m1,m2,m3,0,mgi,eps1,un1,ui1); eyz[mcmax1]=eps1[0];}
                        }
                    }
                    else
     ...
     */
#pragma omp parallel private(n1,m1,m2,m3,mgi,mcmax0,eps1,un1,ui1,ival,ival1) shared(bondm,pbondsum,pbondnum,pstoksum,pstoksum1,pstoksum2,pstoknum,pcontsum,pcontnum,pcontsumb,pcontnumb) firstprivate(mgx,mgy,mgz,mgxy,mgp,maxvxyz)
    {
        /* Obtain cur thread number */
        n1=omp_get_thread_num();
        /* Check Initial Error */
        ...
        /*
         */
#pragma omp for schedule(static) reduction(+:pbondsum10,pbondnum10,pstoksum10,pstoksum11,pstoksum21,pstoknum10,pcontsum10,pcontnum10,pcontsumb10,pcontnumb10)
        for (m3=0;m3<mgz[mgi];m3++)
            for (m1=0;m1<mgx[mgi];m1++)
                for (m2=0;m2<mgy[mgi];m2++)
                {
    ...
    if (printmod)
    {
        printf("\n FILE %s KRUG %d Cycle %ld TIME %e yr  CHANGEbeg %e yr  CHANGEend %e yr\n",fl0out[f0],m0+1,m2prn,timesum/(365.25*24.0*3600.0),timebeg/(365.25*24.0*3600.0),timeend/(365.25*24.0*3600.0));
    ...
    if(0==0 || stoksum2>STOKSMIN*STOKSMAX || stoksum*stoksum1>STOKSMIN*STOKSMAX || contsum>DIVVMIN)
    {
        ...
        for(mgi=1;mgi<=multinum;mgi++) ronurestrict(mgi);
        /**/
        /**/
        /**/
        ...
        /*
         */
#pragma omp parallel for shared(sol0,num0) private(m1) firstprivate(multinum,mgp,mgn) schedule(static)
        for (m1=0;m1<(mgp[multinum]+mgn[multinum])*4;m1++)
        {
            sol0[m1]=0;
            num0[m1]=0;
        }
...
#pragma omp parallel for shared(sol0,pr,vx,vy,vz) private(m1,m2,m3,mcmax0,mcmax1) firstprivate(mgi,mgp,mgn,mgy,mgx,mgz,mgxy) schedule(static)
            ...
            #pragma omp parallel for shared(a,ja) private(m1) firstprivate(nodenum) schedule(static)
            for(m1=0;m1<(mgn[multinum]*57);m1++)
                    {
                    a[m1]=0;
                    ja[m1]=0;
                    }
            #pragma omp parallel for shared(ia) private(m1) firstprivate(nodenum) schedule(static)
            for(m1=0;m1<(mgn[multinum]*4);m1++)
                    {
                    ia[m1]=0;
                    }

            int mp,mvx,mvy,mvz,nn1;
            double mukoef = 1.0;
            // system("rm *.txt");
            for(mgi=0;mgi<=multinum;mgi++)
            {
//#pragma omp parallel for shared(bondm) private(m1,m2,m3,n1,nn1,mcmax,mcmax1,mcmax2,pos0cur1,eps1,un1,ui1,ival) firstprivate(mgi,multinum,mgp,mgx,mgy,mgz,mgxy,timesum,timebeg,timeend,velocitykf,mukoef) schedule(static)
                for (m3=0;m3<mgz[mgi];m3++)
                    for (m1=0;m1<mgx[mgi];m1++)
                        for (m2=0;m2<mgy[mgi];m2++)
                        {
                            ...
                            mcmax=mcmax1*4;
                            mcmax2 = mcmax;
                            if (mgi==multinum) mcmax=(mcmax1-mgp[mgi])*4;
                            /* Pos P,Vx,Vy,Vz in val1[] */
                            pos0cur1=mcmax1*57;
                            if (mgi==multinum) pos0cur1=(mcmax1-mgp[mgi])*57;
                            /*
                            ...
                            if(m1 && m2 && m3)
                            {
                                ...
                                else{ // coarsest multigrid level
                                    if(!bondm[mcmax2+0])
                                    {
                                        /* Check Pressure in Cell calculation Y/N */
                                        continerr(m1,m2,m3,0,mgi,eps1,un1,ui1);
                                        for(nn1=1;nn1<=un1[0];nn1++) un1[nn1]-= mgp[mgi]*4;
                                        pardisomat(1,mcmax+0,mcmax2+0,pos0cur1,n1,un1,ui1);
                                    }
                                    else
                                    {
                                        /* Add P-Boundary */
                                        bonderrpar(mcmax+0,mcmax2+0,0,eps1,un1,ui1);
                                        pardisomat(1,mcmax+0,mcmax2+0,pos0cur1,n1,un1,ui1);
                                        fl = fopen("Pb_pardiso.txt","a");
                                        if (mgi=multinum ) {
                                            for (m4=0; m4 < un1[0]; m4++) {
                                              fprintf(fl,"%ld %ld %ld %e %i \n",m1,m2,m3,ui1[m4],un1[m4]);
                                            }
                                        }
                                        fclose(fl);
                                    }
                            ...
                            if(m2<mgy[mgi]-1 && m3<mgz[mgi]-1)
                            {
                                ...
                                if (mgi!=multinum){
                                    ...
                                    else
                                    {
                                        ...
                                        /* Change velocity */
                                        if(timesum>timebeg && timesum<timeend)
                                        {
                                            /*
                                             ival=(timesum-timebeg)/(timeend-timebeg);
                                             ui1[0]*=(1.0-ival+velocitykf*ival);
                                             */
                                        }
                                        else
                                        {
                                            if(timesum>=timeend) ui1[0]*=velocitykf;
                                        }
                                        gausmat2(1,mcmax+1,pos0cur1+6,un1,ui1);
                                    }
                                } else { // coarsest multigrid level
                                    if(!bondm[mcmax2+1])
                                    {
                                        ...
                                        xstokserr(m1,m2,m3,0,mgi,eps1,un1,ui1);
                                        for(nn1=1;nn1<=un1[0];nn1++) un1[nn1]-= mgp[mgi]*4;
                                        pardisomat(1,mcmax+1,mcmax2+1,pos0cur1+6,n1,un1,ui1);
                                    }
                                    else
                                    {
                                        /* Add vX-Boundary */
                                        bonderrpar(mcmax+1,mcmax2+1,0,eps1,un1,ui1);
                                        /* Change velocity */
                                        if(timesum>timebeg && timesum<timeend)
                                        {
                                            /*
                            ...
                            if(m1<mgx[mgi]-1 && m3<mgz[mgi]-1)
                            {
                                ...
                                if (mgi!=multinum) {
                                    ...
                                    if(!bondm[mcmax2+2])
                                    {
                                        ...
                                        ystokserr(m1,m2,m3,0,mgi,eps1,un1,ui1);
                                        for(nn1=1;nn1<=un1[0];nn1++) un1[nn1]-= mgp[mgi]*4;
                                        pardisomat(1,mcmax+2,mcmax2+2,pos0cur1+23,n1,un1,ui1);

                                    }
                                    else
                                    {
                                        /* Add vX-Boundary */
                                        bonderrpar(mcmax+2,mcmax2+2,0,eps1,un1,ui1);
                                        /* Change velocity */
                                        if(timesum>timebeg && timesum<timeend)
                                        {
                                            /*
                            ...
                            else
                            {
                                    ...
                                    if (mgi=multinum ) {
                                        for (m4=0; m4 < un1[0]; m4++) {
                                          fprintf(fl,"%ld %ld %ld %e %i n",m1,m2,m3,ui1[m4],un1[m4]);
                                        }
                                    }
                                    fclose(fl);
                                }
                            ...
                            if(m1<mgx[mgi]-1 && m2<mgy[mgi]-1)
                            {
                                ...
                                { // coarsest multigrid level
                                    if(!bondm[mcmax2+3])
                                    {
                                        ...
                                        zstokserr(m1,m2,m3,0,mgi,eps1,un1,ui1);
                                        for(nn1=1;nn1<=un1[0];nn1++) un1[nn1]-= mgp[mgi]*4;
                                        pardisomat(1,mcmax+3,mcmax2+3,pos0cur1+40,n1,un1,ui1);

                                    }
                                    else
                                    {                                        /* Add vX-Boundary */

                                        bonderrpar(mcmax+3,mcmax2+3,0,eps1,un1,ui1);
                                        /* Change velocity */
                                        if(timesum>timebeg && timesum<timeend)
                                        {
                                            /*
        ...
        printf("Assembling matrices: done\n");
        if(0==0)
        {
            /* Eliminate empty lines */
            system("rm ia_MG.txt ja_MG.txt");
            m3=0;
            for(m1=1;m1<(mgn[multinum]*57);m1++)
            {
              if(!a[m1]) m3++;
                else
                {
                    a[m1-m3]=a[m1];
                    ja[m1-m3]=ja[m1];
                    if(ja[m1]<0){printf("negative ja in %i",m1);getchar();}
                }
            }

             printf("\n printing ja...\n ");
             fl = fopen("ja_MG.txt","at");
             for(m1=0;m1<(mgn[multinum]*57);m1++)
             {
                fprintf(fl,"%i %i %e\n",m1+1,ja[m1], a[m1]);
             }
            ...
            ia[0]=1;m3=0;
            fl = fopen("ia_MG.txt","at");
            fprintf(fl,"%i %i\n",ia[0],ia[0]);
            for(m1=1;m1<=mgn[multinum]*4;m1++)
            {
              fprintf(fl,"%i %i\n",ia[m1],ia[m1-1]+ia[m1]);
              ia[m1]=ia[m1-1]+ia[m1];
            }
        ...
        printf("Building sparse matrices: done\n"); //exit(0);
        mcmax    = mgp[multinum]+mgn[multinum];
        pos0cur1 = mcmax*57;
        mcmax1   = mcmax*4;
        ...
        do
        {
            ...
            for(n1=0;n1<multicyc;n1++)
            {
                ...
                if(multinum>0)
                {
                    ...
                    /* Downstepping: finest -> coarsest-1 */
                    for(n2=0;n2<multinum;n2++)
                    {
                        ...
                        // if (mgi==multinum) {
                        //   system("rm sol0_prerestrict.txt");
                        //   fl = fopen("sol0_prerestrict.txt","a");
                        //   printf("%i %i\n",mgp[mgi]*4,(mgp[mgi]+mgn[mgi])*4); getchar();
                        //   for (m1=mgp[mgi]*4;m1<(mgp[mgi]+mgn[mgi])*4;m1++)
                        //    {
                        //       // if (m1==117221) {
                        //         // printf("%i %e %e %e \n",m1,sol0[m1],fre1[m1],fre0[m1]); getchar();
                        //       // }
                        //        fprintf(fl,"%i %e %e %e \n",m1,sol0[m1],fre1[m1],fre0[m1]);
                        //    }
                        //   fclose(fl); exit(0);
                        // }
                    ...
                    // system("rm sol0_prePardiso.txt");
                    // fl = fopen("sol0_prePardiso.txt","a");
                    // for (m1=mgp[multinum]*4;m1<(mgp[multinum]+mgn[multinum])*4;m1++)
                    //  {
                    //      fprintf(fl,"%e %e\n",sol0[m1],fre1[m1]);
                    //  }
                    // fclose(fl);
                    // printf("#### 1: mgi %i GAUS SOLUTION PRINTED \n",mgi); getchar();

                    /* Coarsest Level --> direct solver */
                    int mc = 0;
                    system("rm rhs_pardiso.txt");
                    fl = fopen("rhs_pardiso.txt","at");
                    for (m1=mgp[multinum]*4;m1<=(mgp[multinum]+mgn[multinum])*4;m1++)
                    {
                      b[mc] = fre1[m1];
                      fprintf(fl,"%e %i \n",b[mc],m1);
                    ...
                    mgi=multinum;
                    printf("\n Entering pardiso... Mesh size: %i x %i x %i || %i\n",mgx[multinum],mgy[multinum],mgz[multinum], multinnn[1][multinum]);
                    // pardisomat(0,mgn[multinum]*4,0,0,n1,un1,ui1);
                    // vpprolong(multinum,0);
                    printf("\n  Pardiso done \n");
                    gausmat4(multinnn[1][multinum],(mgp[multinum]+mgn[multinum])*4-1,mgp[multinum]*4,multinum);
                    fl = fopen("sol0_coarsest.txt","w");
                    for (m1=mgp[multinum]*4;m1<(mgp[multinum]+mgn[multinum])*4;m1++)
                     {
                         fprintf(fl,"%e %e\n",sol0[m1],fre1[m1]);
                     }
                    ...
                    /* Upstepping: coarsest-1 -> finest -1 */
                    for(n2=multinum-1;n2>0;n2--)
                      {
                      mgi=n2;
                      if(multinnn[0][mgi]>0){
                        system("rm sol0_Gaus_postPardiso.txt");
                        fl = fopen("sol0_Gaus_postPardiso.txt","a");
                        for (m1=mgp[mgi]*4;m1<(mgp[mgi]+mgn[mgi])*4;m1++)
                         {
                             fprintf(fl,"%e %e\n",sol0[m1],fre1[m1]);
                         }
                        fclose(fl);printf("#### 2: mgi %i GAUS SOLUTION PRINTED \n",mgi); getchar();
                    ...
                    /* Finest MG level */
                    mgi=0;
                    system("rm sol0_Gaus_prefinalPardiso.txt");
                    fl = fopen("sol0_Gaus_prefinalPardiso.txt","a");
                    for (m1=mgp[mgi]*4;m1<(mgp[mgi]+mgn[mgi])*4;m1++)
                     {
                         fprintf(fl,"%e %e\n",sol0[m1],fre1[m1]);
                     }
                    fclose(fl);
                    printf("#### 3: mgi %i GAUS SOLUTION PRINTED \n",mgi);
                    ...
                    //           mc++;
                    //         }
                    //         fclose(fl);printf("Done");
                    //
                    //         printf("\n Entering pardiso... Mesh size: %i x %i x %i \n",mgx[mgi],mgy[mgi],mgz[mgi]);  getchar();
                    //         pardisomat(0,mgn[multinum]*4,0,0,n1,un1,ui1);
                    //         printf("\n  Pardiso done \n"); getchar();
                    //     } else {
                    //
                    ...
                    //         // fclose(fl);printf("Done\n");
                    //
                    //        printf("\n Gaus solver... %i %i Mesh size: %i x %i x %i \n",mgi,multinnn[0][mgi],mgx[mgi],mgy[mgi],mgz[mgi]);
                    //        gausmat4(multinnn[0][mgi],(mgp[mgi]+mgn[mgi])*4-1,mgp[mgi]*4,mgi);
                    //          //gausmat4_debug(multinnn[1][mgi],(mgp[mgi]+mgn[mgi])*4-1,mgp[mgi]*4,mgi);
                    //
                    //       //   system("rm sol0_Gaus.txt");
                    //       //   fl = fopen("sol0_Gaus.txt","a");
                    //       //   for (m3=0;m3<mgz[multinum];m3++)
                    //       //       for (m1=0;m1<mgx[multinum];m1++)
                    //       //           for (m2=0;m2<mgy[multinum];m2++)
                    //       //           {
                    //       //               /* Pos in vx[], vy[], vz[], pr[], etc */
                    //       //               mcmax1=(m3*mgxy[multinum]+m1*mgy[multinum]+m2)*4;
                    //       //              fprintf(fl,"%e \n", sol0[mgp[multinum]+mcmax1+0]);
                    //       //              fprintf(fl,"%e \n", sol0[mgp[multinum]+mcmax1+1]);
                    //       //              fprintf(fl,"%e \n", sol0[mgp[multinum]+mcmax1+2]);
                    //       //              fprintf(fl,"%e \n", sol0[mgp[multinum]+mcmax1+3]);
                    //       //
                    //       //            }
                    //       //            fclose(fl);
                    //       //
                    //       // printf("#### GAUS SOLUTION PRINTED \n"); getchar();
                    //
                    //     }
                    //     vpprolong(mgi,0);
                    //     if(printmod) printf("Down%d/%d ",n2,multinnn[1][mgi]);
                ...
                else
                {
                    ...
                    {
                        gausmat4(multinnn[0][mgi],(mgp[mgi]+mgn[mgi])*4-1,mgp[mgi]*4,mgi);
                        if(printmod) printf("Simple UP%d/%d ",mgi,multinnn[0][mgi]);
                    }
                    else
                    {
                        if(multinnn[1][mgi]>0) gausmat4(multinnn[1][mgi],(mgp[mgi]+mgn[mgi])*4-1,mgp[mgi]*4,mgi);
                        if(printmod) printf("Simple DOWN%d/%d ",mgi,multinnn[1][mgi]);
                    }
                }
                if(printmod) printf("\n");
            }
                    ...
                    for (m1=0;m1<mgx[mgi];m1++)
                        for (m2=0;m2<mgy[mgi];m2++)
                        {
                            ...
                            if(m1 && m2 && m3)
                            {
                                ...
                                minpr11=MINV(minpr11,pr[mcmax1]);
                                maxpr11=MAXV(maxpr11,pr[mcmax1]);
                                if(pr[mcmax1]>1e+12) {printf("Pressure is too big %e  at x=%e y=%e z=%e",pr[mcmax1],mggx[mgi][m1],mggy[mgi][m2],mggz[mgi][m3]); exit(0);}
                                /*
                                 printf("Pr %ld %ld %ld %e    %e \n",m1,m2,m3,ival,pr[mcmax1]);
                                 printf("Pr %ld %ld %ld %e    %e",m1,m2,m3,ival,pr[mcmax1]); getchar();
                                 */
...
#pragma omp for schedule(static)
            ...
            /*
             ...
             for (m2=0;m2<mgy[mgi];m2++)
             {
             mcmax1=mgp[mgi]+m3*mgxy[mgi]+(mgx[mgi]-1)*mgy[mgi]+m2;
             mcmax0=mgp[mgi]+m3*mgxy[mgi]+m2;
             {printf("%d %ld %ld %e %e",mgi,mcmax0,mcmax1,sxx[mcmax0],sxx[mcmax1]); getchar();}
             {printf("%d %ld %ld %e %e",mgi,mcmax0,mcmax1,syy[mcmax0],syy[mcmax1]); getchar();}
             {printf("%d %ld %ld %e %e",mgi,mcmax0,mcmax1,szz[mcmax0],szz[mcmax1]); getchar();}
             }
             */
            /**/
            /* Max Vx,Vy,Vz Diff in Grid Calc */
            ...
            /* Parallel region ----------------------------------------------------- */
            //#pragma omp parallel private(n1,m1,m2,m3,mgi,mcmax0,eps1,un1,ui1,ival,ival1,pbondsum10,pbondnum10,pcontsum10,pcontnum10,pcontsumb10,pcontnumb10,pstoksum10,pstoksum11,pstoksum21,pstoknum10) shared(bondm,pbondsum,pbondnum,pstoksum,pstoksum1,pstoksum2,pstoknum,pcontsum,pcontnum,pcontsumb,pcontnumb) firstprivate(mgx,mgy,mgz,mgxy,maxvxyz)
#pragma omp parallel private(n1,m1,m2,m3,mgi,mcmax0,eps1,un1,ui1,ival,ival1) shared(bondm,pbondsum,pbondnum,pstoksum,pstoksum1,pstoksum2,pstoknum,pcontsum,pcontnum,pcontsumb,pcontnumb) firstprivate(mgx,mgy,mgz,mgxy,maxvxyz)
            {
                /*
                 */
                /* Obtain cur thread number */
...
#pragma omp for schedule(static) reduction(+:pbondsum10,pbondnum10,pstoksum10,pstoksum11,pstoksum21,pstoknum10,pcontsum10,pcontnum10,pcontsumb10,pcontnumb10)
            ...
            if (printmod && printmod<=m1prn)
            {
                ...
                /*
                 getchar();
                 */    if(isnan(minvx)||isnan(maxvx)||isnan(minvy)||isnan(maxvy)||isnan(minvz)||isnan(maxvz))
                 {
                     printf("NAN at KRUG %d, iteration cycle %ld of file %d\n",m0+1,m2prn,f0);
                     exit(0);
                 }
            }
            /**/
    ...
    {
        mgi=0;
        /* Set normal stress, normal strain rate and pressure of the first column equal to those of the last one */
        for(m3=0;m3<mgz[mgi];m3++)
            for(m2=0;m2<mgy[mgi];m2++)
            {
                m1=mgp[mgi]+m3*mgxy[mgi]+m2;
                m4=m1+(mgx[mgi]-1)*mgy[mgi];
                sxx[m1]=sxx[m4];
                syy[m1]=syy[m4];
                szz[m1]=szz[m4];
                exx[m1]=exx[m4];
    ...
    if(yperiodic)
    {
        ...
        for(m3=0;m3<mgz[mgi];m3++)
            for(m1=0;m1<mgx[mgi];m1++)
            {
                m2=mgp[mgi]+m3*mgxy[mgi]+m1*mgy[mgi];
                m4=m2+mgy[mgi]-1;
                sxx[m2]=sxx[m4];
                syy[m2]=syy[m4];
                szz[m2]=szz[m4];
    ...
    if(zperiodic)
    {
        ...
        for(m1=0;m1<mgx[mgi];m1++)
            for(m2=0;m2<mgy[mgi];m2++)
            {
                m3=mgp[mgi]+m1*mgy[mgi]+m2;
                m4=m3+(mgz[mgi]-1)*mgxy[mgi];
                sxx[m3]=sxx[m4];
                syy[m3]=syy[m4];
                szz[m3]=szz[m4];
                exx[m3]=exx[m4];
...
#pragma omp parallel
    ...
    /*
     */
#pragma omp parallel for shared(sol1,nu,ro) private(m1) firstprivate(mgi,mgp,mgn) schedule(static)
...
#pragma omp parallel for shared(Sol1,Nu,Ro) private(tid,m1) firstprivate(nt,mgi,mgp,mgn) schedule(static)
    ...
    /*
     */
#pragma omp parallel for shared(Sol1,Nu,Ro) private(tid,m1,m2,m3,m4,m10,m20,m30,m40,m10min,m20min,m30min,dx,dy,dz,ddx,ddy,ddz,swt,ival2) firstprivate(mgi,mgp,mgx,mgy,mgz,mgxy,mggx,mggy,mggz,viscmod) schedule(static)
    ...
    /* Calc New Ro, Nu */
    /*
     #pragma omp parallel for shared(Sol1,Nu,Ro,sol1,nu,ro) private(tid,m1) firstprivate(nt,mgi,mgp,mgn) schedule(static)
     */
    //#pragma omp parallel for shared(Sol1,Nu,Ro,sol1,nu,ro) private(tid,m1) firstprivate(nt,mgi,mgp,mgn) schedule(static) collapse(2)
    for(tid=0;tid<nt;tid++)
    {
#pragma omp parallel for
        for (m1=mgp[mgi];m1<(mgp[mgi]+mgn[mgi]);m1++)
        {
            ...
            nu[m1]+=Nu[tid][m1];
            ro[m1]+=Ro[tid][m1];
            //if(m1==1025){printf("RONU %d %d %ld %ld %e %e %e %e %e %e",mgp[mgi],tid,m1,m2,nu[m1],Nu[tid][m2],ro[m1],Ro[tid][m2],sol1[m1],Sol1[tid][m2]);getchar();}
        }
    }
    /*
     */
#pragma omp parallel for shared(sol1,nu,ro) private(m1) firstprivate(mgi,mgp,mgn,viscmod) schedule(static)
    for (m1=mgp[mgi];m1<(mgp[mgi]+mgn[mgi]);m1++)
    {
        if(isnan(ro[m1])||isnan(nu[m1])){printf("RONU %ld %e %e %e",m1,nu[m1],ro[m1],sol1[m1]);getchar();}
        nu[m1]/=sol1[m1];
        ...
        for (m1=0;m1<mgx[mgi-1];m1++)
            for (m2=0;m2<mgy[mgi-1];m2++)
            {
                ...
                if(m2<mgy[mgi-1]-1 && m3<mgz[mgi-1]-1)
                {
                    ...
                    /*
                     printf("Vx %ld %ld %ld   %ld %ld %ld  %e %e %e %e %e %e %e %d",m1,m2,m3,m10min,m20min,m30min,dx,dy,dz,mggx[mgi][m10min+1],mggx[mgi][m10min],mggy[mgi][m20min+2],mggy[mgi][m20min],mgi);getchar();
                     */
...
{
    /* Counters */
    int m1,m2,m3,m4,m10,m20,m30,m40,m50,m10min,m20min,m30min;
    /* Buffers */
...
#pragma omp parallel for shared(sol1,fre1) private(m1) firstprivate(mgi,mgp,mgn) schedule(static)
    ...
    for(tid=0;tid<nt;tid++)
    {
#pragma omp parallel for shared(Sol0,Sol1) private(m1) firstprivate(tid,nt,mgi,mgp,mgn) schedule(static)
        for (m1=mgp[mgi]*4;m1<(mgp[mgi]+mgn[mgi])*4;m1++)
        {
            Sol0[tid][m1]=0;
            Sol1[tid][m1]=0;
    ...
    /*
     */
#pragma omp parallel for shared(fre0,nd,bondm,Sol0,Sol1) private(tid,m1,m2,m3,m4,m10,m20,m30,m40,m10min,m20min,m30min,dx,dy,dz,ddx,ddy,ddz,swt,ival2) firstprivate(p1koef,v1koef,mgi,mgp,mgx,mgy,mgz,mgxy,mggx,mggy,mggz) schedule(static)
    for (m3=0;m3<mgz[mgi-1];m3++)
        for (m1=0;m1<mgx[mgi-1];m1++)
            for (m2=0;m2<mgy[mgi-1];m2++)
            {
                        ...
                        for (m10=m10min;m10<=m10min+1;m10++)
                        {
                            ...
                            for (m20=m20min;m20<=m20min+1;m20++)
                            {
                                ...
                                if(!bondm[m40*4+1])
                                {
                                     ...
                                     */
                                    /* Add Wt for current node */
                                    Sol0[tid][m40*4+1]+=ival2;
                                    Sol1[tid][m40*4+1]+=swt;
                                }
                            }
                        }
                    }
                        ...
                        for (m10=m10min;m10<=m10min+1;m10++)
                        {
                            ...
                            for (m20=m20min;m20<=m20min+1;m20++)
                            {
                                ...
                                if(!bondm[m40*4+2])
                                {
                                     ...
                                     */
                                    /* Add Wt for current node */
                                    Sol0[tid][m40*4+2]+=ival2;
                                    Sol1[tid][m40*4+2]+=swt;
                                }
                            }
                        }
                    }
                        ...
                        for (m10=m10min;m10<=m10min+1;m10++)
                        {
                            ...
                            for (m20=m20min;m20<=m20min+1;m20++)
                            {
                                ...
                                if(!bondm[m40*4+3])
                                {
                                     ...
                                     */
                                    /* Add Wt for current node */
                                    Sol0[tid][m40*4+3]+=ival2;
                                    Sol1[tid][m40*4+3]+=swt;
                                }
                            }
                        }
                    }
    ...
    for(tid=0;tid<nt;tid++)
    {
#pragma omp parallel for shared(Sol0,Sol1,fre1,sol1) private(m1) firstprivate(tid,nt,mgi,mgp,mgn) schedule(static)
        //#pragma omp parallel for
        for (m1=mgp[mgi]*4;m1<(mgp[mgi]+mgn[mgi])*4;m1++)
        {
            fre1[m1]+=Sol0[tid][m1];
 ...
 #pragma omp parallel for shared(sol0,sol1,fre1) private(m1) firstprivate(mgi,mgp,mgn) schedule(static)
    for (m1=mgp[mgi]*4;m1<(mgp[mgi]+mgn[mgi])*4;m1++)
    {
        ...
        {
            fre1[m1]/=sol1[m1];
            //if(mgi==multinum){printf("%i %e %e\n",m1,sol1[m1],fre1[m1]); getchar();}
            sol1[m1]=0;
        }
        sol0[m1]=0;
