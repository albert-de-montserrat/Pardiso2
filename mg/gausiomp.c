...
/*
#pragma omp barrier
		printf("Thread %d    %ld %ld %ld %e \n",n1,m1,m2,lin0[pos0[m1]+m2],val1[pos0[m1]+m2]);getchar();
*/
		/* Define smallest index */
		m6=MINV(m4,m6);
		}
/*
printf("Thread %d    %ld %ld %ld %ld %ld %e \n",n1,m1,m6,num0[m1],leftn,rightn,fre0[m1]);getchar();
for (m2=m6;m2<m1+rightn+1;m2++) 
		{
		printf("Thread %d    %ld %e \n",n1,m2,buf0[m2]);getchar();
		}
*/
	/**/
...
/*
			#pragma omp flush(pos0)
printf("Thread %d    %ld %ld %ld   %e %e %e\n",n1,m1,m2,m3,ival,*(buf1+m3),*(val0+m8));getchar();
...
/*
	#pragma omp flush(val0)
if(mcmax1>40000) {printf("BEG Thread %d    %ld  %e %e \n",n1,m1,ival,*(fre0+m1));getchar();}
	*(fre0+m1)/=ival;
if(mcmax1>40000) {printf("END Thread %d    %ld  %e %e \n",n1,m1,ival,*(fre0+m1));getchar();}
printf("Thread %d    %ld %ld   %e %e %e\n",n1,m1,m2,ival,*(buf1+m2),*(val0+m4));getchar();
	#pragma omp flush(pos0,val0,fre0)
*/
	}
	#pragma omp flush(pos0)
	*(pos0+m1)=-1;
...
/*
printf("Thread %d    %ld %ld   %e %e %e\n",n1,m1,m2,ival,*(sol0+m2),*(val0+m4));getchar();
