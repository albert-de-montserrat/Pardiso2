...
*/
	...
	if(xperiodic && m1==0) ival=(kt[v[1]]+kt[v[3]])/2.0*xkf/(mggx[mgi][mgx[mgi]-1]-mggx[mgi][mgx[mgi]-2]);
	else ival=(kt[v[1]]+kt[v[3]])/2.0*xkf/(mggx[mgi][m1]-mggx[mgi][m1-1]);
...
*/
	...
	/* dQx/dX=(Qx35-Qx13)/(dX135)=((k13)*(T3-T1)/(dX13) - (k35)*(T5-T3)/(dX35))/(dX135) */
	if(xperiodic && m1==0) ival=(kt[v[1]]+kt[v[3]])/2.0*xkf/(mggx[mgi][mgx[mgi]-1]-mggx[mgi][mgx[mgi]-2])/rocpkf;
	else ival=(kt[v[1]]+kt[v[3]])/2.0*xkf/(mggx[mgi][m1]-mggx[mgi][m1-1])/rocpkf;
...
double ui1[MAXPOS];
/* Err koef */
double heatsum=0,heatnum=0,bondsum=0,bondnum=0,mro,mkt,mcp,mtk,rocpkf,dtk,dtk1,mintk,maxtk;
/**/
/**/
#pragma omp parallel
...
/**/
/* Ro, Cp "Restriction" from Finer to Coarcer level */
for(mgi=1;mgi<=multinumt;mgi++) rocprestrict(mgi);
...
for(n1=0;n1<multicyct;n1++)
	...
	if(multinumt>0)
		{
		/* Up cylcle */
		for(n2=0;n2<multinumt;n2++)
			{
			mgi=n2;
			gausmatt(multittt[0][mgi],mgp[mgi]+mgn[mgi]-1,mgp[mgi]);
			trestrict(mgi+1);
			if(printmod) printf("Up%d/%d ",n2,multittt[0][mgi]);
			}
		/* Down cycle */
		for(n2=multinumt;n2>0;n2--)
			{
			mgi=n2;
			gausmatt(multittt[1][mgi],mgp[mgi]+mgn[mgi]-1,mgp[mgi]);
			tprolong(mgi,0);
			if(printmod) printf("Down%d/%d ",n2,multittt[1][mgi]);
			}
		}
	/* Single cylcle */
	else
...
/*
*/
#pragma omp parallel for shared(markx,marky,markz,markt,markk,Sol0,Sol1,tk,tk0,tk1,tk2,cp,kt,ro) private(tid,n1,ival,ival2,m3,m4,eps1,wn1,wi1,mkt,mcp,mro,mtk,rocpkf,dtk,dtk1) firstprivate(heatdif,marknum,xsize,ysize,zsize,zdeep,tdeep,timestep,gx,gy,gz) schedule(static)
for (m3=0;m3<=marknum;m3++) 
/* Check markers out of grid */
...
/*
*/
#pragma omp parallel for shared(markx,marky,markz,markt,markk,sol1,tk,tk0,tk1,tk2) private(n1,ival,m3,m4,eps1,wn1,wi1) firstprivate(marknum,xsize,ysize,zsize,gx,gy,gz) schedule(static)
	for (m3=0;m3<=marknum;m3++) 
	/* Check markers out of grid */
	if (markt[m3]<50 && markx[m3]>0 && marky[m3]>0 && markz[m3]>0 && (double)(markx[m3])<xsize && (double)(marky[m3])<ysize && (double)(markz[m3])<zsize)
		{
...
/* T "Prolongation" from coarser to finer level */
void tprolong(int mgi, int ynreset)
...
*/
	/* Reset Temperature if required */
	if(ynreset) sol0[m4]=0;
	ival=0;
	...
	for (m30=m30min;m30<=m30min+1;m30++)
		...
		for (m10=m10min;m10<=m10min+1;m10++)
			...
			for (m20=m20min;m20<=m20min+1;m20++)
				...
				ddy=1.0-ddy;
				/* Pos in tk[] for the coarcer level */
				m40=mgp[mgi]+m30*mgxy[mgi]+m10*mgy[mgi]+m20;
				/* Calc wt */
				swt=ddx*ddy*ddz;
/*
if(ddx<0 || ddy<0 || ddz<0 || ddx>1.0 || ddy>1.0 || ddz>1.0) {printf("T %ld %ld %ld   %ld %ld %ld %ld  %e %e %e %e %e",m1,m2,m3,m10,m20,m30,m40,ddx,ddy,ddz,swt,sol0[m4]);getchar();}
printf("Vx %ld %ld %ld   %ld %ld %ld %ld  %e %e %e %e %e",m1,m2,m3,m10,m20,m30,m40,ddx,ddy,ddz,swt,tk[m40]);getchar();
*/
				/* Add T for current node */
				sol0[m4]+=sol0[m40]*swt*t1koef;
...
/*
printf("T  %ld %ld %ld %ld  %e %e",m1,m2,m3,m4,ival,tk[m4]);getchar();
*/
/**/
/*
...
/* End T "Prolongation" from coarser to finer level */
...
/* T residuals "Restriction" from finer to coarser level */
void trestrict(int mgi)
/* mgi - Coarser multigrid level number */
{
/* Counters */
long int m1,m2,m3,m4,m10,m20,m30,m40,m10min,m20min,m30min;
...
for (m2=0;m2<mgy[mgi-1];m2++)
	...
	dz=mggz[mgi][m30min+1]-mggz[mgi][m30min];
/*
printf("T  %ld %ld %ld   %ld %ld %ld  %e %e %e %ld",m1,m2,m3,m10min,m20min,m30min,dx,dy,dz,bondm[nodenum8+m4]);getchar();
*/
	/* Reset Temperature */
	ival=0;
	/* Interpolate from 8 nodes */
	for (m30=m30min;m30<=m30min+1;m30++)
		...
		for (m10=m10min;m10<=m10min+1;m10++)
			...
			for (m20=m20min;m20<=m20min+1;m20++)
				...
				m40=mgp[mgi]+m30*mgxy[mgi]+m10*mgy[mgi]+m20;
				if(!bondm[nodenum8+m40])
					{
					/* Calc wt */
					swt=ddx*ddy*ddz;
/*
if(ddx<0 || ddy<0 || ddz<0 || ddx>1.0 || ddy>1.0 || ddz>1.0) {printf("T  %ld %ld %ld   %ld %ld %ld %ld  %e %e %e %e %e",m1,m2,m3,m10,m20,m30,m40,ddx,ddy,ddz,swt,sol1[m40]);getchar();}
*/
					/* Add T  for current node */
					fre1[m40]+=fre0[m4]*swt;
					/* Add Wt for current node */
					sol1[m40]+=swt;
					}
				}
...
/* End T residuals "Restriction" from finer to coarser level */





/* Ro, Cp, kt "Restriction" from Finer to Coarser level */
void rocprestrict(int mgi)
/* mgi - Coarser multigrid level number */
{
/* Counters */
long int m1,m2,m3,m4,m10,m20,m30,m40,m10min,m20min,m30min;
/* Buffers */
double dx,dy,dz,ddx,ddy,ddz,swt;
/**/
/**/
/**/
/* Clear Old Ro, Cp, kt */
...
for (m2=0;m2<mgy[mgi-1];m2++)
	{
	/* Pos in ro[] cp[] kt[] for the finer level */
	m4=mgp[mgi-1]+m3*mgxy[mgi-1]+m1*mgy[mgi-1]+m2;
	...
	for (m30=m30min;m30<=m30min+1;m30++)
		...
		for (m10=m10min;m10<=m10min+1;m10++)
			...
			for (m20=m20min;m20<=m20min+1;m20++)
				...
				ddy=(mggy[mgi][m20]-mggy[mgi-1][m2])/dy;
				ddy=1.0-ABSV(ddy);
				/* Pos in ro[] cp[] kt[] for the coarcer level */
				m40=mgp[mgi]+m30*mgxy[mgi]+m10*mgy[mgi]+m20;
				/* Calc wt */
				swt=ddx*ddy*ddz;
/*
...
*/
				/* Add Ro, Cp Kt for current node */
if(0==0 || ABSV(1.0-swt)<1e-5)
{
				cp[m40]+=cp[m4]*swt;
				kt[m40]+=kt[m4]*swt;
				ro[m40]+=ro[m4]*swt;
...
/* End Ro, Cp, kt "Restriction" from Finer to Coarser level */
