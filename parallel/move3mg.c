...
if(xperiodic && m1==0) xkf=2.0/(mggx[mgi][1]-mggx[mgi][0]+mggx[mgi][mgx[mgi]-1]-mggx[mgi][mgx[mgi]-2]);
...
/* ynerr - Err Calc Y(1)/N(0) */
{
    ...
    {
        /* Add Const */
        leftb=sol0[mcmax2]-bondv1[bondm[mcmax2]][0];
        /* Add Koef */
        if(bondn1[bondm[mcmax2]])
        {
            // leftb-=bondv1[bondm[mcmax2]][1]*sol0[bondn1[bondm[mcmax2]]-1 - mgp[multinum-1]*4];
            leftb-=bondv1[bondm[mcmax2]][1]*sol0[bondn1[bondm[mcmax2]]-1 - mgp[multinum-1]*4];
        }
        /*
         printf("Bo %ld %e %e \n",mcmax,sol0[mcmax],leftb);
    ...
    if(bondn1[bondm[mcmax2]])
    {
        ...
        if(un1[un1[0]]<1){
            printf(" 2: %i %i %i %ld \n",mcmax,mcmax2,bondm[mcmax2],un1[un1[0]]);
...
if (m2prn==0) { // THIS CONDITION SHOULD BE ACTIVE ONLY AND IF ONLY ALL THE RHEOLOGIES ARE LINEAR (i.e. stiffness matrix doesn't change during Piccard iterations)
/**  RESET ja AND a **/
#pragma omp parallel for shared(a,ja,ia) private(m1) firstprivate(nodenum) schedule(static)
for (m1 = 0; m1 < mgn[multinum]*57; m1++) {
  ja[m1] = 0;
  a[m1]  = 0.0;
  if(m1 < mgn[multinum]*4){
    ia[m1] = 0;
  }
}
int nn1;
...
/*
*/
// #pragma omp parallel for shared(bondm) private(m1,m2,m3,mcmax,mcmax1,pos0cur1,eps1,un1,ui1,ival) firstprivate(mgi,multinum,mgp,mgx,mgy,mgz,mgxy,timesum,timebeg,timeend,velocitykf) schedule(static)
  ...
  if(m1 && m2 && m3)
  {
      ...
      if (mgi!=multinum) {
          ...
          if(!bondm[mcmax2+0])
          {
              /* Check Pressure in Cell calculation Y/N */
              continerr(m1,m2,m3,0,mgi,eps1,un1,ui1);
              // fl = fopen("Pcoefs_par.txt","a");
              for(nn1=1;nn1<=un1[0];nn1++){
              //   fprintf(fl,"%e %i %i \n",ui1[nn1],un1[nn1],un1[nn1]- mgp[mgi]*4);
                un1[nn1]-= mgp[mgi]*4;
              }
              // fclose(fl);
...
for(n1=0;n1<multicyc;n1++)
  ...
  */
        ...
        // if (mgi==multinum-1){
        //   fl = fopen("sol0_prerestrict.txt","w");
        //   for (m1=mgp[mgi]*4;m1<(mgp[mgi]+mgn[mgi])*4;m1++)
        //    {
        //       fprintf(fl,"%e %e %e \n",sol0[m1],fre1[m1],fre0[m1]);
        //    }
        //   fclose(fl); //exit(0);
        //   // printf("\n#### 0: mgi %i GAUS SOLUTION PRINTED \n",mgi); getchar();
        // }
    ...
    if (m2prn==0 ){
      ...
      /* Coarse Level --> direct solver */
        ...
        printf("\n Entering pardiso... Mesh size: %i x %i x %i \n",mgx[multinum],mgy[multinum],mgz[multinum]);
...
*/
			if(multinnn[1][mgi]>0){
				...
				// printf("\n Gaus solver... %i %i Mesh size: %i x %i x %i \n",mgi,multinnn[1][mgi],mgx[mgi],mgy[mgi],mgz[mgi]); getchar();
			}
			vpprolong(mgi,0);
...
if (m3==am-1 && mgi==multinum-1) {
    fl = fopen("fre1_par_gausmat4.txt","w");
  }
...
{
/* Counters */
long int m1,m2,m3,m4,m10,m20,m30,m40,m50,m10min,m20min,m30min;
/* Buffers */
