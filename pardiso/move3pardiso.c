...
/* ynerr - Err Calc Y(1)/N(0) */
{
    ...
    if (ynerr==1)
    {
        ...
        if(bondn1[bondm[mcmax2]])
        {
            ...
            leftb-=bondv1[bondm[mcmax2]][1]*sol0[bondn1[bondm[mcmax2]]-1];
        }
        /*
         printf("Bo %ld %e %e \n",mcmax,sol0[mcmax],leftb);
         */
      ...
      if(multinum>0)
      {
        ...
        for(n2=0;n2<multinum;n2++)
        {
          ...
          */
          ...
          fl = fopen("sol0_gaus_fine.txt","w");
          for (m1=mgp[mgi]*4;m1<(mgp[mgi]+mgn[mgi])*4;m1++)
          // for (m1=mgp[0]*4;m1<(mgp[0]+mgn[0])*4;m1++)
          {
            fprintf(fl,"%e %e\n",sol0[m1],fre1[m1]);
          }
          fclose(fl);
        ...
        fl = fopen("sol0_precoarsest.txt","w");
        // for (m1=mgp[mgi]*4;m1<(mgp[mgi]+mgn[mgi])*4;m1++)
        for (m1=mgp[0]*4;m1<(mgp[0]+mgn[0])*4;m1++)
        {
          fprintf(fl,"%e %e\n",sol0[m1],fre1[m1]);
          ...
          */
          ...
          if(multinnn[1][mgi]>0){
            ...
            if (mgi==multinum) {
              ...
              fclose(fl);
              printf("#### 2: mgi %i GAUS SOLUTION PRINTED \n",mgi); getchar();
            }
          ...
          fl = fopen("sol0_coarsest_prolonged.txt","w");
          for (m1=mgp[mgi-1]*4;m1<(mgp[mgi-1]+mgn[mgi-1])*4;m1++)
          {
            fprintf(fl,"%e %e\n",sol0[m1],fre1[m1]);
          }
          fclose(fl);
